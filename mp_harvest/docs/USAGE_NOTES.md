...
## 2.5 导出全部正文到指定目录（2026-08-09）

- 历史页点「导出到目录…」：输入目标目录（支持 `~`，不存在会自动创建），
  将**当前视图全部**正文导出为逐篇 HTML，并在目录内生成 `index.html` 说明页
  （titles_filtered 风格：关键字筛选、表头排序、判定/本地正文/原文链接、暗色适配）。
- 目录记忆在浏览器 localStorage（`mp_harvest.export_dir`）；留空则用后端默认
  `data/exports/<账号>/`。导出是后台任务，进度在任务列表/toast 可见，完成后
  toast 显示实际目录。
