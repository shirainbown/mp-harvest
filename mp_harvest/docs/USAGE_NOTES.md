...
- **逐篇文件命名**：`日期_公众号名_编号_标题.html`（如
  `2026-08-05_半导体行业观察_01_标题.html`；缺日期/公众号时自动跳过该段，
  非法字符统一替换为 `_`）。
- **自动识别公众号名（2026-08-09）**：名称留空（默认「未命名公众号」）时，
  拉取历史成功后会用 getmsg 返回的官方昵称自动改名（前端账号列表同步刷新）；
  **手动输入的自定义名称不会被覆盖**。
- **批量拉取（2026-08-09）**：历史页「批量拉取…」勾选多个公众号一次拉取
  （聚合任务按账号报进度，可取消；到期未续约的账号不可勾选）。
- **全部公众号视图与排序（2026-08-09）**：公众号下拉选「全部公众号」可看所有
  账号的文章合并列表（新增公众号列）；排序支持按时间（同日按公众号名）或按名称
  （组内按时间新→旧）。
- **异常退出代理自愈（2026-08-09）**：若抓包中应用被强杀导致系统代理残留在
  127.0.0.1:8088，下次启动会自动检测并关闭该残留代理（端口已死时），
  不会出现「一开机全机断网」；若已在断网状态，可手动把系统代理改回
  127.0.0.1:7897（Clash）或关闭代理。
