...
def test_storage_endpoint_shape(client, auth):
    b = client.get("/api/storage", params=auth).json()
    assert set(b) == {"items", "clearable_bytes", "total_bytes"}
    keys = {i["key"] for i in b["items"]}
    # 可清理的
    assert {"update", "events", "ai_title_cache", "weekly_cache"} <= keys
    # 不可清理的也要露脸（让用户看见「这些不归清理管」）
    assert {"accounts", "ai_models", "prompts", "articles_cache"} <= keys
